...
const scoresData = JSON.parse(fs.readFileSync(
  path.join(__dirname, '../../test_data_2/student_scores_final.json'),
  'utf8'
));
...
// Prepare INSERT statements with VALUES for batch insert
const values = [];
...
scoresData.students.forEach((student) => {
  ...
  student.interventions.forEach((intervention) => {
    ...
    if (!interventionId || intervention.intervention_name === 'Capstone') return;
    ...
    Object.keys(intervention.microcompetency_scores).forEach((mcCode) => {
      ...
      values.push(
        ...
        `'${teacherId}', NOW(), '', 'Submitted', '${termId}')`
      );
    });
  });
});
...
console.log(`📊 Generated ${batches.length} batch INSERT statements`);
console.log(`📊 Total scores: ${values.length}`);
console.log(`\n✅ Ready to execute batches\n`);

// Write to file
const outputFile = path.join(__dirname, '../../test_data_2/batch_insert_scores.sql');
fs.writeFileSync(outputFile, batches.join('\n\n'));
console.log(`💾 SQL batches written to: ${outputFile}`);

// Export for execution
module.exports = { batches, totalScores: values.length };
